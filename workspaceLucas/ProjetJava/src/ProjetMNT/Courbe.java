...
public class Courbe {
	private List<Point> points;	//Liste de points triÃ©s 
	...
	public static List<Courbe> planAltimetrique(List<Point> points, MNT modele)	{
		/**Le but de cette mÃ©thode est de diffÃ©rencier plusieurs courbes de niveau de mÃªme altitude Ã  partir d'un nuage de point,
		 * Tout d'abord il faut trier la liste donnÃ©e en argument, 
		...
		while(liste_triee.size() < taille_liste)	{
			...
			liste_triee.add(pointPlusProche);	//On choisit ce point 
												//car Ã©tant le plus proche du dernier point de la liste triÃ©e
			points.remove(pointPlusProche);	//Et on le supprime de la liste pour ne pas le re-choisir
		}
		//Elimination des doublons dans la liste triÃ©e
		...
		if(continuer)	{	//Si pas de distance trop grande rencontrÃ©e
	...
	public static void main(String[] args) {	//Debug
		...
		for(int i=0; i<courbes.size(); i++)	{
			...
			System.out.println("Courbe numÃ©ro: " + i);
