...
public class Point {
	...
	public Point(Point point1, Point point2, double altitude)	{
		...
		if ((altitude <= point1.getZ() & altitude >= point2.getZ()) | (altitude <= point2.getZ() & altitude >= point1.getZ()))	{	
			...
			double difference_petite = altitude - pointMin.getZ();	//Diff�rence d'altitude entre point le plus bas et altitude souhait�e
			double distance_petite   = difference_petite * distance / difference_altitude;	//Distance entre point le plus bas et le future point � l'altitude souhait�e
			if(pointMin.getX() == pointMax.getX()) {
				//System.out.println("X �gaux");
				...
				if ((pointMin.getX() - pointMax.getX())/distance >= 0)	{	///////////////Erreur ici, sûrement au niveau de l'angle alors que dans l'autre condition ça marche
	...
	public static Point plusProche(Point point, List<Point> liste)	{
		...
		int index      = 0;	//Variable qui garde en m�moire l'emplacement dans la liste
							//du point le plus proche du dernier point de la liste tri�e
